...
Bundle.prototype.open = function(done) {
    ...
    return FS.exists(self.path, function(exists) {
	    ...
	    return FS.open(self.path, "wx+", function(err, fd) {
	    	if (err) return callback(err);
...
Bundle.prototype.save = function(callback) {
    ...
    function generateCode(callback) {
        ...
        function writePayload() {
            code.push('// @sourcemint-bundle-ignore: \nsourcemint.bundle("", function(require)\n{\n');

            code.push('// @sourcemint-bundle-header: ' + JSON.stringify(self.header[1]));
            code.push(self.header[0]);

            ITERATOR(self.module).forEach(function(value, key) {
                code.push('// @sourcemint-bundle-module: ' + JSON.stringify(value[1]));
                code.push('require.memoize("' + key + '", ');
                code.push(value[0]);
                code.push(');');
            });

            ITERATOR(self.descriptors).forEach(function(value, key) {
                code.push('// @sourcemint-bundle-descriptor: ' + JSON.stringify(value[1]));
                code.push('require.memoize("' + key + '", ');
                code.push(value[0]);
                code.push(');');
            });
        ...
        function writeFooter() {
            code.push('// @sourcemint-bundle-report: ' + JSON.stringify(self.report) + '\n');
        }
