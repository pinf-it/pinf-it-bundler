...
//		var sandboxes = {};
		var modules = {};
//console.log("getExistingModules", moduleObj);
		function forModule(moduleObj) {


//				sandboxes[sandboxId] = true;

console.log("moduleObj.bundle", moduleObj.bundle);

			var bundleDescriptor = bundleDescriptorForId(moduleObj.bundle);

//console.log("bundleDescriptor", bundleDescriptor);
...
console.log("bundleDescriptor", bundleDescriptor);
	...
	//console.log("memoizeId", memoizeId);	
						modules[memoizeId] = bundleDescriptor.modules[memoizeId];// || bundleDescriptor.expectExistingModules[memoizeId];
				...
				// We stop now that we have collected all modules from the sandbox
				// we found based on the bundle our calling module was contained within.
//				return;
...
//console.log("EXISTING", getExistingModules(moduleObj));

				var existingModules = getExistingModules(moduleObj);
/*
				var packageDescriptor = (
					(existingModules[moduleObj.pkg + "/package.json"] && existingModules[moduleObj.pkg + "/package.json"].descriptor) ||
					bundleDescriptor.modules[moduleObj.pkg + "/package.json"].descriptor
				);
*/
...
console.log("moduleObj.pkg", moduleObj.pkg);
...
//console.log("bundleDescriptors", bundleDescriptors);					
//console.log("moduleObj.bundle", moduleObj.bundle);					
...
//_getBundleDescriptor

console.log("packageDescriptor", packageDescriptor);
				...
				bundleOptions.onRun(bundlePath, {
					...
					resolveDynamicSync: function(moduleObj, pkg, sandbox, canonicalId, options) {
						...
						if (/^\//.test(canonicalId)) {
							...
							var mappingInfo = getMappingInfo(moduleObj, alias);							
							...
							var distFilename = options.normalizeIdentifier((mappingInfo.pkg + ((id) ? "/" + id : "")).replace(/\//g, "+"));
...
console.log("**id", id);
								var options = {
									...
									rootModule: (id) ? "./" + options.normalizeIdentifier(id) : "",
...
console.log("filePath", filePath);
								...
								var deferred = Q.defer();
								BUNDLER.bundlePackage(filePath, options, function(err, subDescriptor) {									
									if (err) return deferred.reject(err);
									...
									return BUNDLER.augmentBundle(mappingInfo.bundleDescriptor.bundle.path, options, function(err, bundle, done) {
										if (err) return deferred.reject(err);

console.log("bundle.modules", bundle.modules);
console.log("moduleObj.pkg", moduleObj.pkg);
