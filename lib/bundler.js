	...
	function bundleFile(bundleFilePath, callback) {
		...
		function getPackageDescriptorForPath(path, options, callback) {
			...
			return loadPackageDescriptorForPath(path, options, function(err, packageDescriptor) {
				...
				// TODO: Add an `options.chrootPath` option to make paths relative which will ensure
				//		 that package IDs do not change if installed at different root paths.
				shasum.update(path);
				packageDescriptor.id = "";
				if (path != bundleFilePath) {
					packageDescriptor.id = shasum.digest("hex") + "-" + PATH.basename(path);
				}
				...
				if (
					...
					packageDescriptor.memoized.main = packageDescriptor.combined.exports.main.replace(/^\./, "");
				}
				if (
				...
				function resolvePackageId(id, callback) {
					function checkDescriptor(packageDescriptor, callback) {
						...
						function resolve(path) {
							...
							return FS.exists(PATH.join(path, id), function(exists) {
								if (exists) {
									return getPackageDescriptorForPath(
										PATH.join(path, id),
										options,
		...
		return getPackageDescriptorForPath(bundleFilePath, options, function(err, packageDescriptor, packageExtras) {
			...
			opts.finalizeBundle = function(bundleDescriptor, bundle, callback) {
				for (var packagePath in packages) {
					if (Object.keys(packages[packagePath][0].memoized).length > 0) {
						bundleDescriptor.modules["/package.json"] = {
							requireId: "/package.json",
							memoizeId: "/package.json",
							wrapper: "json"
						};
						bundle.setModule(
							packages[packagePath][0].id + "/package.json",
							JSON.stringify(packages[packagePath][0].memoized),
							{
								file: null,
								mtime: 0,
								wrapper: "json",
			...
			return exports.bundleFile(PATH.join(bundleFilePath, packageDescriptor.combined.exports.main), opts, function(err, bundleDescriptor) {
				if (err) return callback(err);

				var bundleId = packageDescriptor.combined.exports.main.replace(/^\.\//, "/");
				...
				bundleDescriptor.warnings.forEach(function(warning) {
					bundlePackageDescriptor.warnings.push([].concat(warning, "file", packageDescriptor.combined.exports.main));
				});
				bundleDescriptor.errors.forEach(function(error) {
					bundlePackageDescriptor.errors.push([].concat(error, "file", packageDescriptor.combined.exports.main));
				});
	...
	try {
		...
		function init(bundleFilePath, callback) {
			bundleDescriptor = {
				modules: {},
				bundle: {
					path: PATH.join(options.distPath, PATH.basename(bundleFilePath))
				},
				warnings: [],
				errors: []
			};
		...
		function addFile(filePath, options, callback) {
			...
			return MODULE_INSIGHT.parseFile(filePath, options, function(err, moduleDescriptor) {
				...
				if (options.existingModules && options.existingModules[descriptor.requireId]) {
				...
				return WRAPPER.wrapModule(descriptor, options, function(err) {
					...
					bundleDescriptor.modules[descriptor.requireId] = descriptor;
			...
			return addFile(filePath, options, function(err, descriptor) {
				...
				var waitfor = WAITFOR.serial(function(err) {
					...
					descriptor.warnings.forEach(function(warning) {
						bundleDescriptor.warnings.push([].concat(warning, "module", descriptor.requireId));
					});
					descriptor.errors.forEach(function(error) {
						bundleDescriptor.errors.push([].concat(error, "module", descriptor.requireId));
					});
					return callback(null, descriptor);
				});
		...
		return realpath(bundleFilePath, function(err, bundleFilePath) {
			...
			return init(bundleFilePath, function(err) {
				...
				return openBundle(bundleDescriptor.bundle.path, function(err) {
					...
					opts.id = "/" + PATH.basename(bundleFilePath).replace(/\.js/, "");
					...
					return bundleStaticallyLinkedFiles(bundleFilePath, opts, function(err, moduleDescriptor) {
						...
						var waitfor = WAITFOR.serial(function(err) {
							...
							if (!options.existingModules || !options.existingModules["/package.json"]) {

								if (!bundleDescriptor.modules["/main"] && !bundleDescriptor.modules["/package.json"]) {
									bundleDescriptor.modules["/package.json"] = {
										requireId: "/package.json",
										memoizeId: "/package.json",
										wrapper: "json"
									...
									bundle.setModule(
										normalizeExtension("/package.json"), JSON.stringify({
