...
exports.bundleFile = function(bundleFilePath, options, callback) {
	try {
		...
		options.packageDescriptorForModule = function(path, callback) {
			return PACKAGE_INSIGHT.findPackagePath(options._realpath(path), function(err, path) {
				...
				return PACKAGE_INSIGHT.parse(path, options, function(err, packageDescriptor) {
					...
					function copyExports (callback) {
						if (!firstTime) {
							return callback(null);
						}
						var waitfor = WAITFOR.serial(function (err) {
							if (err) {
								console.error("Exports copy error", err[0].stack);
								return callback(err);
							}
						...
						if (
							packageDescriptor.combined.exports &&
							packageDescriptor.combined.exports.images
						) {
							Object.keys(packageDescriptor.combined.exports.images).forEach(function (route) {
								var sourcePath = PATH.join(options._realpath(packageDescriptor.dirpath), packageDescriptor.combined.exports.images[route]);
								var targetPath = PATH.join(options._realpath(bundleDescriptor.bundle.path).replace(/\.js$/, ""), packageDescriptor.id, route);
								...
								if (
									...
									waitfor(function (done) {
										console.log("Copy export", sourcePath.replace(/\/\*$/, ""), "to", targetPath.replace(/\/\*$/, ""));
									...
									waitfor(function (done) {
										console.log("Copy export", sourcePath, "to", targetPath);
					...
					return copyExports(function (err) {
						if (err) return callback(err);
		...
		function resolveModuleId(descriptor, id, options, callback) {
			...
			if (/^\./.test(id)) {
				return options.API.FS.exists(PATH.join(descriptor.descriptor.filepath, "..", memoizeId.replace(/\.js$/, "")), function(nonJsModuleExists) {

					if (nonJsModuleExists) {

						return options.API.FS.stat(PATH.join(descriptor.descriptor.filepath, "..", memoizeId.replace(/\.js$/, "")), function(err, stat) {
							if (err) return callback(err);
							...
							if (stat.isDirectory()) {
								return options.API.FS.exists(PATH.join(descriptor.descriptor.filepath, "..", memoizeId.replace(/\.js$/, ""), "package.json"), function(exists) {
									...
									return options.API.FS.exists(PATH.join(descriptor.descriptor.filepath, "..", memoizeId, "index.js"), function(exists) {
										...
										return callback(null, PATH.join(descriptor.descriptor.filepath, "..",  memoizeId.replace(/\.js$/, "") + ext), PATH.join(descriptor.requireId, "..", memoizeId.replace(/\.js$/, "") + ext));
									});
								});
							} else {
