...
//console.log("aliasedPackagePath",options._realpath(aliasedPackagePath));
						...
						return resolvePackageId(idParts[0], function(err, aliasedPackagePath, aliasdPackageDescriptor, packageExtras) {
							...
							try {
								...
								if (idParts.length > 1) {
									...
									memoizeId = PATH.join((aliasdPackageDescriptor.id || idParts[0]) + aliasdPackageDescriptor.combined.exports.main.replace(/^\.|\.js$/, ""));
								}
...
//			console.log("addDynamic", path, memoizeId, "opts.packageExtras.packagePath", opts.packageExtras && opts.packageExtras.packagePath, new Error().stack);
	...
	//console.log("resolved module id", path, memoizeId, "packageExtras.packagePath", packageExtras && packageExtras.packagePath);
