...
exports.bundleFile = function(bundleFilePath, options, callback) {
	try {
		...
		function addStatic(path, memoizeId, callback) {
		...
		function addDynamic(path, memoizeId, callback) {
			var opts = {};
			for (var key in options) {
				opts[key] = options[key];
			}
			opts.distPath = PATH.join(options.distPath, PATH.basename(bundleFilePath.replace(/\.js$/, "")), PATH.dirname(memoizeId.replace(/^\//, "")));
			return exports.bundleFile(path, opts, callback);
		}
		...
		return realpath(bundleFilePath, function(err, bundleFilePath) {
			...
			return init(bundleFilePath, function(err) {
				...
				return openBundle(bundleDescriptor.bundle.path, function(err) {
					...
					return bundleStaticallyLinkedFiles(bundleFilePath, {
						id: "/" + PATH.basename(bundleFilePath).replace(/\.js/, "")
						...
						if (options.forceMemoizeFiles) {
							for (var memoizeId in options.forceMemoizeFiles) {
								waitfor(memoizeId, function(memoizeId, done) {
									addStatic(options.forceMemoizeFiles[memoizeId], memoizeId.replace(/\.js/, ""), done);
								});
							}
						}
						waitfor();
